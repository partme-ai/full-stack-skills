...
> **标准说明**：本文由 原「视觉与交互 DNA」阶段模板全文 全文合并。

### 关联文档

| 文档 | 说明 |
| :--- | :--- |
| `{V}/7、{Name}-UI设计说明-{V}.md` | 版本 UI 总说明 |
| `10、{Name}-功能菜单与版本规划.md` | 菜单 |
...
- **高效**：体现产品的高效性和便捷性
- **友好**：体现产品的友好性和易用性
- **陪伴与成长**：{例如：partme-ai的设计不仅仅是工具，更是孩子的伙伴。设计语言应体现：趣味性、安全感、仪式感、简洁性}
...
**同步策略**：
- **毫秒级反馈**：{例如：家长在App修改设置后，硬件端需在3s内展示同步动效（如partme-ai点头并说"收到啦"）}
...
## 附录 A：控制台类产品 DNA 示例（OctoPanel 原文节选，结构对齐用）

> 文件名保留历史名，仅用于兼容旧引用。正文产品名统一为 **{Name}**。

## 1. 设计目标

{Name} 的界面不是聊天应用，也不是业务工作台，而是 **1Panel 风格的多引擎控制台**。

设计目标：

- 专业、稳定、可扫描
- 对新手友好，对运维场景可执行
- 统一承载安装、实例、节点、任务、审计、扩展等页面
- 保持自有品牌视觉，不沿用旧“旧客服感配色”

## 2. 核心设计原则

- **平台优先**：导航与页面结构以控制台为中心
- **状态优先**：健康、任务、风险、回滚等状态必须比装饰更醒目
- **路径清晰**：列表、详情、向导三种页面模式要稳定
- **操作克制**：主操作固定、危险操作突出、日志与上下文就近可见
...
| 名称 | 色值 | 用途 |
|------|------|------|
| Octo Navy | `#0F172A` | 顶栏、侧栏、标题深色 |
| Octo Blue | `#2563EB` | 主按钮、链接、选中态 |
| Octo Teal | `#14B8A6` | 平台高亮、进度、次主操作 |
| Octo Sky | `#E0F2FE` | 轻背景、信息区块 |
| Octo Amber | `#F59E0B` | 引擎强调、警告、向导高亮 |
...
- **概览页**：Dashboard、节点总览、巡检总览
- **列表页**：Instances、Nodes、Channels、Models、Tasks
- **详情页**：实例详情、节点详情、任务详情
- **向导页**：安装向导、连接器安装、扩展安装

### 4.3 Desktop 跟随

Desktop 复用相同页面语义，差异只在：
...
- **1Panel 式控制台骨架**
- **自有蓝青深色平台配色**
- **四类固定页面范式：概览 / 列表 / 详情 / 向导**
- **右侧上下文抽屉 + 任务抽屉** 作为核心交互层
